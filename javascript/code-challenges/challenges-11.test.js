...
function transformToLis(obj){
 ...
 let keys = Object.keys(  obj ) ;
  ...
  // console.log(`<li>${keys[1]}: ${values[1]}</li>`) ;

  // (`<li>name: bob</li>`);
  if( keys[0])  {
  output.push(`<li>${keys[0]}: ${values[0]}</li>`) ; 
  output.push(`<li>${keys[1]}: ${values[1]}</li>`) ; 
  }
  ...
  for(let i in input){
     let temp=[] ;
      for(let j in input[i]){
        ...
        // console.log(input[i][j]);
        input[i][j] % 5 === 0 ? temp.push( 2**input[i][j]) : '' ; 
        }
...
let output='';
  ...
  for(let i in data){
    if (data[i].gender === 'female' | data[i].gender === 'male' ){
        ...
        output === '' ? output= data[i].name : output =output + ` and ${data[i].name}`;
...
let output='';
 ...
 let shortVal =data.sort( (a,b) =>{
   console.log(  a.height ) ;
   return( a.height-b.height );
   
 } ) ;

    return(shortVal[0].name); 
  }
