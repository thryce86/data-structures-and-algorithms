...
const sortByChildren = ((charArray) => { 
  ...
  var sortedArray = charArray.sort(  (a,b)  =>{
// console.log(b.children.length);

    if (b.children.length===a.children.length){
      console.log(  ( b.name>a.name) ? 1 : -1    )  ;
      let temp = ( b.name<a.name) ? 1 : -1 ;
      return(temp);
    }else{
      return(
...
const isCapitalized = (input) => {
  ...
  let temp2 = temp.map((xyz) => {
    let out =   xyz.split(')')[0].replace(',','')  ;
    // console.log( out) ;
    return out
  });
...
let outputArray=[] ;
   temp.forEach((inny)=>{
    let conditional = regex.test(inny);
    // console.log(inny ,conditional ) ;
    if (conditional){
      outputArray.push(inny) ;
    }
